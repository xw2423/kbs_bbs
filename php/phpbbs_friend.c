...
{   
    ...
    i=0;
    sethomefile(fpath, userid, "ignores");
        
    if ((fn=fopen(fpath, "r")) < 0)
        RETURN_FALSE;
    fseek(fn, (IDLEN + 1) * start, SEEK_SET);
    while (fread(buf, IDLEN+1, 1, fn) > 0) {
        MAKE_STD_ZVAL(element);
        array_init(element);
        add_assoc_string(element, "ID", buf, 1);
        zend_hash_index_update(Z_ARRVAL_P(return_value), i, (void *) &element, sizeof(zval *), NULL);
        i++;
        if (i>=num)
            break;
...
PHP_FUNCTION(bbs_countignores)
{
    ...
    RETURN_LONG(st.st_size / (IDLEN+1));
}
...
PHP_FUNCTION(bbs_delete_ignore)
{
    ...
    int userid_len;
    int ac = ZEND_NUM_ARGS();
    char buf[STRLEN], ignoreuser[IDLEN+1];
    int deleted;
    ...
    deleted = search_record(buf, ignoreuser, IDLEN+1, (RECORD_FUNC_ARG)cmpinames, userid);

    if (deleted > 0) {
        if (delete_record(buf, IDLEN+1, deleted, NULL, NULL) != -1) {
            RETURN_LONG(0);
        } else {
            RETURN_LONG(3);
        }
...
PHP_FUNCTION(bbs_add_ignore)
{
    ...
    if (get_num_records(buf, IDLEN+1) >= MAX_IGNORE) {
        RETURN_LONG(-1);
    }
    ...
    added = search_record(buf, ignoreuser, IDLEN+1, (RECORD_FUNC_ARG)cmpinames, userid);

    if (added > 0) {
        RETURN_LONG(-2);
    }
    if (append_record(buf, lookupuser->userid, IDLEN+1)==0) {
        RETURN_LONG(0);
    } else {
        RETURN_LONG(-3);
    }
