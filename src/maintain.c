...
#define MB_ITEMS 28
    ...
    /* 多个版主 */
    sel[29].hotkey='U';
    sprintf(menustr[29],"%-15s%s",menuldr[29],(bh.flag&BOARD_SCORE_FUNC)?"是":"否");
#endif	
    /*退出*/
#ifdef NEWSMTH
    ...
    while (loop) {
        ...
        if (ret==SHOW_QUIT) {
            ...
            if (lastkey==KEY_ESC) {
                if (change) {
                    move(20,0); clrtoeol();
                    getdata(22,2,"\033[1;31m放弃修改退出? [N]: \033[m",buf,2,DOECHO,NULL,true);
                    if (buf[0]!='y'&&buf[0]!='Y')
                        continue;
                }
                return -1;
        ...
        switch (currpos-1) {
            ...
            /* 多个版主 */
            case 29:
                newbh.flag^=BOARD_SCORE_FUNC;
                if ((bh.flag&BOARD_SCORE_FUNC)^(newbh.flag&BOARD_SCORE_FUNC)) {
                    sprintf(menustr[29],"%-15s\033[1;32m%s\033[m",menuldr[29],(newbh.flag&BOARD_SCORE_FUNC)?"是":"否");
                    change|=(1<<29);
                } else {
